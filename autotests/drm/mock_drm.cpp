    ...
    : fd(fd)
{
    ...
    for (int i = 0; i < numCrtcs; i++) {
        const auto &plane = QSharedPointer<MockPlane>::create(this, PlaneType::Primary, i);
        crtcs << QSharedPointer<MockCrtc>::create(this, plane, i, gammaSize);
        planes << plane;
    }
...
MockGpu::~MockGpu()
{
    const auto blobs = propertyBlobs;
    qDeleteAll(blobs);
    s_gpus.remove(fd);
}
...
{
    auto it = std::find_if(propertyBlobs.constBegin(), propertyBlobs.constEnd(), [id](const auto &propBlob){
        return propBlob->id == id;
    });
    return it == propertyBlobs.constEnd() ? nullptr : *it;
}
...
MockPropertyBlob::MockPropertyBlob(MockGpu *gpu, const void *d, size_t size)
    ...
    , size(size)
{
    gpu->propertyBlobs << this;
    memcpy(data, d, size);
}

MockPropertyBlob::~MockPropertyBlob()
{
    gpu->propertyBlobs.removeOne(this);
    free(data);
}
...
MockConnector::MockConnector(MockGpu* gpu)
    ...
    , encoder(QSharedPointer<MockEncoder>::create(gpu, 0xFF))
{
    ...
    addProp("non-desktop", 0, DRM_MODE_PROP_IMMUTABLE);
...
{
    auto mode = libxcvt_gen_mode_info(width, height, refreshRate, false, false);

    drmModeModeInfo modeInfo;
    modeInfo.vdisplay = mode->vdisplay;
    modeInfo.hdisplay = mode->hdisplay;
    modeInfo.clock = mode->dot_clock;
    modeInfo.hsync_start = mode->hsync_start;
    modeInfo.hsync_end = mode->hsync_end;
    modeInfo.htotal = mode->htotal;
    modeInfo.vsync_start = mode->vsync_start;
    modeInfo.vsync_end = mode->vsync_end;
    modeInfo.vtotal = mode->vtotal;
    modeInfo.vrefresh = mode->vrefresh;
    modeInfo.flags = mode->mode_flags;

    modeInfo.type = DRM_MODE_TYPE_DRIVER;
    if (preferred) {
        modeInfo.type |= DRM_MODE_TYPE_PREFERRED;
    }
    sprintf(modeInfo.name, "%dx%d@%d", width, height, modeInfo.vrefresh);
...
MockCrtc::MockCrtc(MockGpu* gpu, const QSharedPointer<MockPlane> &legacyPlane, int pipeIndex, int gamma_size)
    : MockObject(gpu)
    , pipeIndex(pipeIndex)
...
int drmIoctl(int fd, unsigned long request, void *arg)
{
    ...
    if (request == DRM_IOCTL_MODE_CREATE_DUMB) {
        ...
        auto dumb = QSharedPointer<MockDumbBuffer>::create(gpu, args->width, args->height, args->bpp);
...
drmModePropertyBlobPtr drmModeGetPropertyBlob(int fd, uint32_t blob_id)
{
    ...
    auto it = std::find_if(gpu->propertyBlobs.constBegin(), gpu->propertyBlobs.constEnd(), [blob_id](const auto &blob){return blob->id == blob_id;});
    if (it == gpu->propertyBlobs.constEnd()) {
...
int drmModeCreatePropertyBlob(int fd, const void *data, size_t size, uint32_t *id)
{
    ...
    MockPropertyBlob *blob = new MockPropertyBlob(gpu, data, size);
    *id = blob->id;
    return 0;
}
...
{
    GPU(fd, -EINVAL);
    for (const auto &blob : qAsConst(gpu->propertyBlobs)) {
        if (blob->id == id) {
            gpu->propertyBlobs.removeOne(blob);
            delete blob;
            return 0;
        }
    }
