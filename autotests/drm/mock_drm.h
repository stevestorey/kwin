...
public:
    ...
    uint32_t type;
    QSharedPointer<MockEncoder> encoder;
...
public:
    MockCrtc(MockGpu *gpu, const QSharedPointer<MockPlane> &legacyPlane, int pipeIndex, int gamma_size = 255);
    ...
    QSharedPointer<MockPlane> legacyPlane;
};
...
public:
    ...
    QVector<QSharedPointer<MockConnector>> connectors;
    QVector<drmModeConnectorPtr> drmConnectors;

    QVector<QSharedPointer<MockEncoder>> encoders;
    QVector<drmModeEncoderPtr> drmEncoders;

    QVector<QSharedPointer<MockCrtc>> crtcs;
    QVector<drmModeCrtcPtr> drmCrtcs;

    QVector<QSharedPointer<MockPlane>> planes;
    QVector<drmModePlanePtr> drmPlanes;

    QVector<MockFb*> fbs;
    QVector<QSharedPointer<MockDumbBuffer>> dumbBuffers;
    QVector<MockPropertyBlob*> propertyBlobs;
